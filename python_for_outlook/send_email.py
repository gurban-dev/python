...
def main():
	...
	try:
		...
		'''
		The Authorization: Bearer {access_token} header validates
		the request by proving the app has permission to access
		emails in Outlook.'''
		headers = {
		...
		'''
		Python list data structure to store the files that are to
		be attached to the email message. The create_attachment()
		function is necessary to invoke because it converts each
		file to an attachment object.'''
		attachments = [
