...
def get_collaborators(ctx, list_name, project_name):
	'''
	Retrieve a SharePoint list object by its title using the
	get_by_title() method of the lists property of the web
	object within the ClientContext.
...
access_token: A string representing the access token obtained from
Microsoft Graph, which is needed to authenticate the request.

collaborators: A list of strings, where each string is an email
address of a project collaborator.
'''

# Step 6: Send emails to collaborators
def send_emails(access_token, collaborators):
	'''
	Assign the Microsoft Graph API endpoint for sending an email.

	The endpoint is actually: /me/sendMail endpoint. Using /me implies
	that the email will be sent on behalf of the user whose account is
	associated with the access token.'''
	graph_url = "https://graph.microsoft.com/v1.0/me/sendMail"

	'''
	The Authorization: Bearer {access_token} header validates the request
	by proving the app has permission to access emails in Outlook.'''
	headers = {
	...
	for email in collaborators:
		...
		if response.status_code == 202:
			# "text" is an attribute of the httpx.Response class that holds
